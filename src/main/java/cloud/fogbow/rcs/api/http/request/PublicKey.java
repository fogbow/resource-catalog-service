...
public class PublicKey {
    ...
    public ResponseEntity<cloud.fogbow.rcs.api.http.response.PublicKey> getPublicKey() throws UnexpectedException {
        try {
            LOGGER.info(Messages.Info.GET_PUBLIC_KEY);
            String publicKeyValue = ApplicationFacade.getInstance().getPublicKey();
            cloud.fogbow.rcs.api.http.response.PublicKey publicKey = new cloud.fogbow.rcs.api.http.response.PublicKey(publicKeyValue);
